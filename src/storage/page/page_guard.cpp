  ...
  if (frame_->is_dirty_) {
    {
      ...
      BUSTUB_ASSERT(is_valid_, "tried to flush an invalid read guard");
      BUSTUB_ASSERT(frame_->page_id_ == page_id_, "Page ID mismatch");
...
void ReadPageGuard::Drop() {
...

  {
    // Take the BPM latch
    // Decrement pin count
    // If pin count reaches 0, make the frame evictable in the replacer
    std::scoped_lock bpm_lock(*bpm_latch_);
...
void WritePageGuard::Flush() {
  if (frame_->is_dirty_) {
    {
      std::scoped_lock bpm_lock(*bpm_latch_);
      BUSTUB_ASSERT(is_valid_, "tried to flush an invalid write guard");
      BUSTUB_ASSERT(frame_->page_id_ == page_id_, "Page ID mismatch");
